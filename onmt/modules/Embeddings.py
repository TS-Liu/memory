...
class RelativePositionEncoding(nn.Module):
    """
    add soft-relative position encoding
    url: https://arxiv.org/abs/1803.02155
    """
    
    def __init__(self, dropout, hidden_size, k=16):
        super(RelativePositionEncoding, self).__init__()
        self.projection = nn.Linear(hidden_size, k)
        self.softmax = nn.Softmax(dim=-1)
        self.position_embedding = nn.Linear(k, hidden_size, bias=False)
        self.input_dropout = nn.Dropout(dropout)

    def forward(self, emb):
        score = self.softmax(self.projection(emb))
        relpos = self.position_embedding(score)
        new_emb = emb + relpos
        return self.input_dropout(new_emb)
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        if position_encoding:
            ...
            # pe = RelativePositionEncoding(dropout, self.embedding_size)
